...
def get_prim_coords(atoms_obj):
    con_dict = connec.bonds_dict(atoms_obj, con_frag=True)
    prim_coords = connec.get_all_ic_edges(con_dict)
    return prim_coords


def get_prim_carts(atoms_obj):
    """ Returns all cartesian vectors for primitivate coordinates
    (bonds, angles, dihedrals) found for given Atoms object
    :param (ase.Atoms) atoms_obj:
    :return (list[np.ndarray]) cartesian_prim_coords:
    """
    posns = atoms_obj.get_positions()
    bond_dict = connec.bonds_dict(atoms_obj, con_frag=True)
    prim_coords = connec.get_all_ic_edges(bond_dict)
    cartesian_prim_coords = []
    for pc in prim_coords:
        ...
        cartesian_prim_coords.append(wilson_pc)
    return cartesian_prim_coords
...
#         trans = ref[1][get_idx]
#         trans_idcs.append(geom.add_trans_idcs(trans_idcs[-1],
#                                               trans))
#     return trans_idcs
...
def get_all_ic_trans_idcs(ics, bond_dict):
    ...
    output = []
    for ic in ics:
        output.append(geom.gen_all_trans_idcs(ic, bond_dict))
    return output
